-- First, check and update existing role values that don't match the new enum
UPDATE public.profiles SET role = 'super_admin' WHERE role = 'superadmin';
UPDATE public.profiles SET role = 'user' WHERE role NOT IN ('super_admin', 'admin', 'user');
...
-- Update the role column to use the new enum (now that values are compatible)
...
DROP POLICY IF EXISTS "Auto insert profile on signup" ON public.profiles;
...
DROP TRIGGER IF EXISTS update_subscription_dates_trigger ON public.profiles;
